...
""", unsafe_allow_html=True)
    ...
    # 1. Load Parameters
    with open(shared_dir / f"hidden_{team}.json") as f:
        hidden_params = json.load(f)

    with open(shared_dir / f"cooperation_{team}.json") as f:
        coop_params_raw = json.load(f)

    with open(shared_dir / f"domestic_{team}.json") as f:
        domestic_event = json.load(f)

    with open(shared_dir / "international.json") as f:
        international_event = json.load(f)  # list of 2 dicts
    ...
    if "paper_count" not in st.session_state:
        st.session_state["paper_count"] = initial_data[team]["papers"]
    if "model_count" not in st.session_state:
        st.session_state["model_count"] = initial_data[team]["models"]
    if "US_papers" not in st.session_state:
        st.session_state["US_papers"] = initial_data["US"]["papers"]
        st.session_state["US_models"] = initial_data["US"]["models"]
    if "China_papers" not in st.session_state:
        st.session_state["China_papers"] = initial_data["China"]["papers"]
        st.session_state["China_models"] = initial_data["China"]["models"]

    # 4. Domestic event evaluation
    ...
    # 5. International event evaluation
    delta_paper_international = sum(
        evaluate_event_international(event["delta_papers"], hidden_params, coop_params_raw)
        for event in international_event
    )
    delta_model_international = sum(
        evaluate_event_international(event["delta_models"], hidden_params, coop_params_raw)
        for event in international_event
    )

    # 6. Update papers
    if team == "US":
        paper_growth_this_round = random.randint(150, 250)
    elif team == "China":
        paper_growth_this_round = random.randint(200, 300)
    else:
    ...
    # 7. Update models (event-based + probabilistic)
    st.session_state["model_count"] += delta_model_domestic + delta_model_international
    st.session_state["model_count"] += calculate_ai_models(st.session_state["paper_count"])
    ...
    st.session_state["US_models"] = int(round(calculate_ai_models(st.session_state["US_papers"])))
